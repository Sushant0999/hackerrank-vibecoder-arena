...
const ASTEROID_SIZE_MIN = 40; // Minimum size for asteroid obstacles
const ASTEROID_SIZE_MAX = 120; // Maximum size for asteroid obstacles
const ASTEROID_SPEED_INITIAL = 80; // Initial speed of asteroids
const ASTEROID_SPAWN_RATE = 0.8; // Base spawn rate of asteroids (per second)
...
const BROKEN_PLANET_SPAWN_RATE = 0.05; // Spawn chance for broken planets (per frame)

const REWARD_SPAWN_RATE = 0.02; // Spawn chance for rewards (per frame)
...
const NEBULA_ANIMATION_FACTOR = 0.0001; // Factor for subtle nebula background animation

const PLAYER_MAX_HEALTH = 100; // Maximum health of the player
const SMALL_COLLISION_DAMAGE = 10; // Damage from small collision
const MEDIUM_COLLISION_DAMAGE = 30; // Damage from medium collision
const STRONG_COLLISION_DAMAGE = 60; // Damage from strong collision
...
let player = {
  ...
  blink: false, blinkTimer: 0, // Used for player invincibility blink effect
};
...
let lastObstacleSpawn = 0; // Timestamp for controlling obstacle spawn rate
...
let environmentTransitionAlpha = 0; // Alpha for fade-in/out effect
let isTransitioning = false; // Flag to indicate if an environment transition is in progress
...
function loadHighestScore() {
  ...
  gameOverHighestScoreDisplay.textContent = Math.floor(highestScore);
}
...
function initializeStars() {
  ...
  for (let i = 0; i < STAR_COUNT; i++) {
    backgroundStars.push({
      ...
      size: getRandomArbitrary(STAR_SIZE_MIN, STAR_SIZE_MAX),
...
function updatePlayerVelocity(dt) {
  ...
  if (Number.isFinite(currentVelMag) && currentVelMag > maxSpeed) {
    player.vx = (player.vx / currentVelMag) * maxSpeed;
    player.vy = (player.vy / currentVelMag) * maxSpeed;
  }
}
...
function gameLoop(timestamp) {
  ...
  if (!gameOver && gameRunning && !isTransitioning) { // Only update if game is running and not transitioning
    update(dt, timestamp); // Update game logic if not game over and game is running
  }
...
function update(dt, timestamp) {
  ...
  const currentAsteroidRate = ASTEROID_SPAWN_RATE + (level - 1) * 0.1; // Asteroid rate increases with level
  const currentBrokenPlanetRate = BROKEN_PLANET_SPAWN_RATE + (level - 1) * 0.01; // Broken planet rate increases with level

  if (lastObstacleSpawn >= 1 / currentAsteroidRate) {
    lastObstacleSpawn = 0;
    spawnObstacle('asteroid');
  }
  if (getRandomArbitrary(0, 1) < currentBrokenPlanetRate * dt) {
    spawnObstacle('brokenPlanet');
  }

  // Update obstacles and handle collisions with bullets
  const newObstacles = [];
  for (let i = 0; i < obstacles.length; i++) {
    const o = obstacles[i];
    o.y += o.speed * dt; // Move obstacle downwards
    
    // Process bullets hitting this obstacle
    if (player.shooter && !o.isDestroyed) { // Only check if obstacle isn't already destroyed
      const remainingBullets = [];
      for (let j = 0; j < bullets.length; j++) {
        const b = bullets[j];
        // Simple circular collision check between bullet and obstacle
        const distSq = (b.x - o.x) * (b.x - o.x) + (b.y - o.y) * (b.y - o.y);
        if (distSq < (o.radius + BULLET_SIZE / 2) * (o.radius + BULLET_SIZE / 2)) {
          ...
          remainingBullets.push(b); // If bullet didn't hit, keep it
        }
      }
      bullets = remainingBullets; // Update bullets array
    }

    // Add obstacles to new list if still active (on screen and not destroyed)
    if (o.y > GAME_HEIGHT + o.radius) {
      // Obstacle went off-screen
      if (!o.isDestroyed) { // Only give bonus if not destroyed by bullet
        score += 50; // Bonus score for avoiding un-destroyed obstacles
      }
    } else if (!o.isDestroyed) { // Keep obstacle if not destroyed and still on screen
      newObstacles.push(o);
    }
  }
  obstacles = newObstacles; // Update with only active obstacles
  ...
  bullets = activeBullets; // Update with only active bullets

  // Reward spawning and collection
  if (getRandomArbitrary(0, 1) < REWARD_SPAWN_RATE * dt) {
    spawnReward();
  }
  ...
  for (let i = 0; i < rewards.length; i++) {
    ...
    const distSq = (player.x + PLAYER_WIDTH / 2 - r.x) * (player.x + PLAYER_WIDTH / 2 - r.x) +
                   (player.y + PLAYER_HEIGHT / 2 - r.y) * (player.y + PLAYER_HEIGHT / 2 - r.y);

    if (r.y > GAME_HEIGHT + r.radius) {
      // Reward went off-screen, remove it
    } else if (distSq < (r.radius + Math.min(PLAYER_WIDTH, PLAYER_HEIGHT) / 2) * (r.radius + Math.min(PLAYER_WIDTH, PLAYER_HEIGHT) / 2)) {
      activateReward(r.type); // Activate reward effect
    } else {
      newRewards.push(r);
    }
  }
  ...
  // Power-up timer management and visual indicator blinking
  // Invincible power-up
  if (player.invincibleTimer > 0) {
    player.invincibleTimer -= dt; // Decrement timer
    player.blinkTimer += dt;
    if (player.blinkTimer >= 0.1) { // Toggle blink state every 0.1s
      player.blink = !player.blink;
      player.blinkTimer = 0;
    }
    // Make indicator blink when power-up is about to expire
    invincibleRewardIndicator.style.opacity = (player.invincibleTimer <= REWARD_DURATION / 3 && Math.floor(player.invincibleTimer * 10) % 2 === 0) ? '0.3' : '1';
    invincibleRewardIndicator.style.display = 'inline-block';
    if (player.invincibleTimer <= 0) {
      player.invincible = false;
      invincibleRewardIndicator.style.display = 'none';
      player.blink = false; // Stop blinking
    }
  } else {
    invincibleRewardIndicator.style.display = 'none';
    player.blink = false;
  ...
  // Shooter power-up
  if (player.shooterTimer > 0) {
    player.shooterTimer -= dt;
    shooterRewardIndicator.style.opacity = (player.shooterTimer <= REWARD_DURATION / 3 && Math.floor(player.shooterTimer * 10) % 2 === 0) ? '0.3' : '1';
  ...
  if (player.turboTimer > 0) {
    ...
    turboRewardIndicator.style.opacity = (player.turboTimer <= TURBO_REWARD_DURATION / 3 && Math.floor(player.turboTimer * 10) % 2 === 0) ? '0.3' : '1';
  ...
  if (!player.invincible) {
    ...
    const newObstaclesInv = []; // Create a new array for remaining obstacles
    for (const o of obstacles) {
      // Check for collision as before but just remove obstacle and give score
      ...
      if (distSq < (o.radius + playerEffectiveRadius) * (o.radius + playerEffectiveRadius)) {
        ...
        newObstaclesInv.push(o); // Keep obstacle if no collision
      }
    }
    obstacles = newObstaclesInv; // Update obstacle list
  }

  // Score and level progression
  score += dt * 10; // Score increases over time
  if (level < ENVIRONMENTS.length && score >= level * 1000) { // Level up every 1000 points
    level++;
    // Trigger smooth transition to new environment
    isTransitioning = true;
    environmentTransitionAlpha = 0; // Start fade out
    currentEnvironmentIndex = (currentEnvironmentIndex + 1) % ENVIRONMENTS.length; // Pre-select next environment
  }

  // Update environment transition alpha for smooth fade
  if (isTransitioning) {
    environmentTransitionAlpha += dt * 0.8; // Fade speed
    if (environmentTransitionAlpha >= 1) { // When fully opaque at 1
      isTransitioning = false; // End transition
      environmentTransitionAlpha = 0; // Reset for next transition
    }
  }
...
function render(dt, timestamp) {
  ...
  if (timeSinceCollision < COLLISION_SHAKE_DURATION && !gameOver) { // Only shake if game is not over
  ...
  // Draw obstacles (asteroids and broken planets) without spinning, only movement
  for (const o of obstacles) {
    ctx.save();
    ...
    const segments = (o.type === 'asteroid') ? getRandomInt(5, 8) : getRandomInt(8, 12);
    // Draw the irregular shape based on fixed random points per obstacle
    for (let i = 0; i < segments; i++) {
        const angle = (Math.PI * 2 / segments) * i;
        // Use pre-calculated random points per obstacle
        const currentRadius = o.randomPoints[i] || o.radius; 
  ...
  // Draw environment transition overlay
  if (isTransitioning) {
    const alpha = Math.sin(environmentTransitionAlpha * Math.PI); // Creates a smooth fade in-out curve
    ctx.save();
...
/**
 * Spawns a new obstacle (asteroid or broken planet) at the top of the screen.
 * Obstacles have irregular shapes and fall downwards without spinning.
 * @param {'asteroid'|'brokenPlanet'} type - The type of obstacle to spawn.
 */
function spawnObstacle(type) {
  ...
  if (type === 'asteroid') {
    radius = getRandomArbitrary(ASTEROID_SIZE_MIN, ASTEROID_SIZE_MAX) / 2;
    speed = ASTEROID_SPEED_INITIAL * getRandomArbitrary(0.8, 1.2) + (level - 1) * 10;
    colors = currentEnvironment.obstacleColor; // Use environment specific colors
    damage = SMALL_COLLISION_DAMAGE;
    health = 1;
  } else { // Broken Planet
    radius = getRandomArbitrary(BROKEN_PLANET_SIZE_MIN, BROKEN_PLANET_SIZE_MAX) / 2;
    speed = ASTEROID_SPEED_INITIAL * getRandomArbitrary(0.5, 0.9) * 0.8 + (level - 1) * 5;
    colors = currentEnvironment.obstacleColor; // Use environment specific colors
    damage = MEDIUM_COLLISION_DAMAGE;
    health = 3;
  }
  ...
  const segments = (type === 'asteroid') ? getRandomInt(5, 8) : getRandomInt(8, 12);
  const randomPoints = [];
  for (let i = 0; i < segments; i++) {
...
function spawnReward() {
  ...
  const radius = 15; // Fixed radius for rewards
  const x = getRandomArbitrary(radius, GAME_WIDTH - radius); // Random X position
...
function checkPlayerObstacleCollisions() {
  for (let i = obstacles.length - 1; i >= 0; i--) {
    ...
    if (distSq < (o.radius + playerEffectiveRadius) * (o.radius + playerEffectiveRadius)) {
      player.lastCollisionTime = globalThis.performance.now() / 1000; // Trigger screen shake
      let damageToApply = o.damage;

      // Adjust damage for broken planets based on their size
      if (o.type === 'brokenPlanet') {
          damageToApply = o.radius > BROKEN_PLANET_SIZE_MAX * 0.75 / 2 ? STRONG_COLLISION_DAMAGE : MEDIUM_COLLISION_DAMAGE; // Corrected radius check
      }
...
function triggerGameOver() {
  ...
  finalScoreDisplay.textContent = Math.floor(score); // Display final score
  gameOverHighestScoreDisplay.textContent = Math.floor(highestScore); // Display highest score on game over screen
  gameOverQuoteDisplay.textContent = GAME_OVER_QUOTES[getRandomInt(0, GAME_OVER_QUOTES.length - 1)]; // Display random quote

  // Reset player state to prevent carry-over
  player.vx = 0; player.vy = 0;
  keys = {}; // Clear pressed keys
  
  // Hide all reward indicators
  invincibleRewardIndicator.style.display = 'none';
  shooterRewardIndicator.style.display = 'none';
  healthRewardIndicator.style.display = 'none';
  turboRewardIndicator.style.display = 'none';
}
...
function startGame() {
  ...
  player.invincibleTimer = 0; player.shooterTimer = 0; player.turboTimer = 0;
  player.health = PLAYER_MAX_HEALTH; player.blink = false;
  player.lastShotTime = 0;
  
  // Clear all entities
  obstacles = []; bullets = []; rewards = [];
  lastObstacleSpawn = 0;
  lastTimestamp = 0; // Reset last timestamp for dt calculation
  keys = {}; // Clear active keys
  
  // Reset UI displays
  scoreDisplay.textContent = '0';
  levelDisplay.textContent = '1';
  healthDisplay.textContent = PLAYER_MAX_HEALTH;
  
  // Hide overlays
  startScreen.style.display = 'none';
  gameOverScreen.style.display = 'none';
  
  // Ensure reward indicators are hidden at start
  invincibleRewardIndicator.style.display = 'none';
  shooterRewardIndicator.style.display = 'none';
  healthRewardIndicator.style.display = 'none';
  turboRewardIndicator.style.display = 'none';
